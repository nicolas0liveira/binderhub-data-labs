# Ruído branco
#install.packages(c("fpp2", "forecast", "tidyverse"))
library(fpp2)
library(forecast)
library(gridExtra)
...
PETR4.SA_clean <- na.omit(PETR4.SA["2015-01-02/2019-04-16"]) # datas limites
PETR4.SA_clean <- PETR4.SA[index(PETR4.SA_clean)]

nrow(PETR4.SA_clean)

#Temos 1065 observações
...
g1 <- autoplot(petr_ts)+ ggtitle("PETR4 - Cotações Obtidas no Yahoo_Finance")

# Autocorrelação
Acf(petr_ts)

# A série é estacionaria? NOPE!
...
g2 <- autoplot(retorno_ts)+title("retorno PETR4")
...
# Vamos aos testes (ainda não vimos isso)
...
#Para os retornos
pp.test(retorno_ts)
#In pp.test(retorno_ts) : p-value smaller than printed p-value. 
#Rejeita Ho. 
