...
ajuste_sazonalidade <- HoltWinters(ts.train, seasonal = "additive")
...
prev_hw <- forecast(ajuste_sazonalidade, h = 36, level=99)
prev_hw
...
autoplot(ts_desemprego) +
        autolayer(fit.HW_addit$fitted, series= "HW_Aditivo",  PI = FALSE)+
...
ts.train <- window(ts_desemprego,  end =  c(2018,6))

ts.test <- window(ts_desemprego, start = c(2018, 7))
...
autoplot(ts.train) +
        ...
        xlab("Data") +
        ylab("Taxa de Desemprego") +
        ggtitle("Taxa de Desemprego RMSP") +
