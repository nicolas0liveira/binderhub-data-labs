...
end <- as.Date("2020-03-02")  

getSymbols('DCOILWTICO', src = 'FRED', from = start, to = end)

DCOILWTICO_clean <- na.omit(DCOILWTICO["2000-02-07/2020-03-02"]) 
...
pnormGC(80, region="below", mean=mean(WALK),
        sd= sd(WALK) ,graph=TRUE)

qnormGC(0.10, region="below", mean=mean(WALK),
        sd= sd(WALK) ,graph=TRUE)
